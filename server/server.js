...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    if (!user.totp_secret) return res.json({ status: 'setup_required', userId: user.id });
...
app.get('/api/audit-logs', async (req, res) => {
  try {
    const result = await pool.query(`SELECT audit_logs.*, users.username, users.real_name FROM audit_logs LEFT JOIN users ON audit_logs.user_id = users.id ORDER BY timestamp DESC`);
...
app.get('/api/policies/:id/documents', async (req, res) => {
    const { id } = req.params;
    try {
        const result = await pool.query('SELECT * FROM policy_documents WHERE policy_id = $1 ORDER BY uploaded_at DESC', [id]);
        res.json(result.rows);
    } catch (err) { res.status(500).send('Server Error'); }
});
...
// Claims (UPDATED: Adds resolved_at)
...
app.patch('/api/claims/:id', async (req, res) => {
  ...
  try {
    ...
    // TIMESTAMP LOGIC: If status is finalized, set resolved_at
...
// Complaints (UPDATED: Adds resolved_at)
...
app.patch('/api/complaints/:id', async (req, res) => {
  ...
  // TIMESTAMP LOGIC: Check if 'status' is changing to 'Resolved'
